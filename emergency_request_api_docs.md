...
```
*Note: `blood_type` must be one of: `'A+' 'A-' 'B+' 'B-' 'AB+' 'AB-' 'O+' 'O-'`. `quantity_required` must be greater than 0. `latitude` and `longitude` are optional during registration but recommended for location-based features. All other fields are required.*
...
Retrieves a list of published emergency requests that are within a **20km radius** of the donor's current location (using PostGIS spatial queries).
...
## 7. Update Donor Location (Donor App)
Allows a logged-in donor to update their current geographic coordinates. This is typically called by the mobile app's geolocator to ensure the donor receives nearby emergency notifications.
